...
export class SessionService implements OnModuleDestroy, OnModuleInit {
  ...
  async findAll(): Promise<Session[]> {
    ...
    // Batch update disconnected sessions
    if (sessionsToUpdate.length > 0) {
      await this.sessionRepository.save(sessionsToUpdate);
      this.logger.log(`Synced ${sessionsToUpdate.length} session(s) status to disconnected`, {
        action: 'status_sync',
